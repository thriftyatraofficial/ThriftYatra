...
async function enforceOTPCooldown(filter) {
    ...
    if (elapsed < OTP_RESEND_WINDOW_MS) {
        return `Wait ${Math.ceil((OTP_RESEND_WINDOW_MS - elapsed) / 1000)}s before requesting another OTP.`;
    }
...
export async function POST(request) {
    try {
        ...
        if (email) {
            ...
            return response(true, 200, 'OTP sent to your email!', { email: cleanEmail });
        }
        ...
        if (phone) {
            ...
            return response(true, 200, 'OTP sent!', { phone: cleanPhone });
        }
