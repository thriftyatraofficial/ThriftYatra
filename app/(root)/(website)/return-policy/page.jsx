...
const breadcrumb = { title: 'Return Policy', links: [{ label: 'Return Policy' }] }
...
const ReturnPolicyPage = () => {
    ...
    const fetchContent = async () => {
        try {
            const { data } = await axios.get('/api/content/return-policy')
        ...
        <div>
            ...
            <div className='lg:px-40 px-5 py-20'>
                {loading ? <div className="text-center py-10">Loading...</div> : content ? (
                    ...
                    <div>
                        <h1 className='text-xl font-semibold mb-3'>Return Policy</h1>
                        <p>Returns accepted only if delivered size differs from listed size.</p>
                    </div>
                )}
            </div>
        </div>
