...
const ThriftSellerOrders = () => {
    ...
    const fetchOrders = async () => {
        try {
            setLoading(true)
            const { data } = await axios.get('/api/seller/orders?type=thrift')
    ...
    return (
        <div className='w-full'>
            <div className='flex justify-between items-center mb-5'>
                <div><h1 className='text-2xl font-bold'>Orders</h1><p className='text-gray-500'>View your thrift orders</p></div>
            </div>
            ...
            <Card>
                ...
                <CardContent>
                    {loading ? <div className='text-center py-10'>Loading...</div> : filteredOrders.length === 0 ? (
                        ...
                        <Table>
                            <TableHeader><TableRow><TableHead>Order ID</TableHead><TableHead>Product</TableHead><TableHead>Date</TableHead><TableHead>Amount</TableHead><TableHead>Status</TableHead><TableHead className='text-right'>View</TableHead></TableRow></TableHeader>
                            <TableBody>
                                {filteredOrders.map((order) => (
                                    <TableRow key={order._id}>
                                        ...
                                        <TableCell className='text-right'><Button variant="ghost" size="icon" onClick={() => router.push(`/seller/thrift/orders/${order.orderId}`)}><Eye className='h-4 w-4' /></Button></TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
...
export default ThriftSellerOrders