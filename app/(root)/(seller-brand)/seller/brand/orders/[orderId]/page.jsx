...
import { ArrowLeft, Package, Truck, Receipt, Layers, CheckCircle } from 'lucide-react'
...
const BrandSellerOrderDetails = () => {
    ...
    return (
        <div className='w-full max-w-3xl mx-auto'>
            ...
            <Card className="mb-6">
                <CardHeader><h2 className='text-lg font-semibold flex items-center gap-2'><Layers className="h-5 w-5" /> Product</h2></CardHeader>
                <CardContent>
                    <div className='flex gap-4'>
                        ...
                        <div className='flex-1 space-y-3'>
                            ...
                            {order.size && order.color && <div className='flex gap-2'><Badge variant="outline">{order.size}</Badge><Badge variant="outline">{order.color}</Badge></div>}
                            <div className='grid grid-cols-3 gap-3'>
                                <div className='bg-gray-50 p-3 rounded text-center'><p className='text-xs text-gray-500'>Price</p><p className='font-semibold'>{formatCurrency(order.unitPrice || order.sellingPrice)}</p></div>
                                ...
                                <div className='bg-gray-50 p-3 rounded text-center'><p className='text-xs text-gray-500'>Total</p><p className='font-semibold'>{formatCurrency(order.totalAmount)}</p></div>
                            </div>
                        </div>
                    </div>
                </CardContent>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
                ...
                <Card>
                    ...
                    <CardContent className='space-y-2'>
                        <div className='flex justify-between'><span className='text-gray-500'>Subtotal</span><span>{formatCurrency(order.subtotal || order.totalAmount)}</span></div>
...
export default BrandSellerOrderDetails